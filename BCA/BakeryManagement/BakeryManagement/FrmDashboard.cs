...
namespace BakeryManagement
{
    public partial class FrmDashboard : Form
    {
        ...
        {/*
            // 1. TOTAL AVAILABLE PRODUCTS
            SqlConnection conn = new SqlConnection(myConn);
            conn.Open();

            SqlCommand cmd1 = new SqlCommand("SELECT COUNT(*) FROM Product", conn);
            int Product = (int)cmd1.ExecuteScalar();
            lblTotalSold.Text = "Total Products: " + Product;
            ...
            // 2. TODAY SALES COUNT
            SqlCommand cmd2 = new SqlCommand("SELECT SUM(Qty) FROM BillDetail WHERE Bill_Date >= @today AND Bill_Date < @tomorrow", conn);
            ...
            int todaySales = (result == DBNull.Value) ? 0 : Convert.ToInt32(result);
            lblTodaysSold.Text = "Today's Sold Items: " + todaySales;

            //TODAY'S SALES COUNT TABLE
            SqlCommand cmd3 = new SqlCommand("SELECT Bill_ID,Product_Name,Qty,Product_Price,Amount,Cust_ID FROM BillDetail WHERE DateValue([Bill_Date])=Date()", conn);
            ...
            // 3. LOW STOCK ALERT
            SqlCommand cmd4 = new SqlCommand("SELECT Product_Name,Qty_Available FROM [Product] WHERE ([Qty_Available]) <= 5", conn);
            ...
            if (lowItems == "")
                ...
                lblLowStock.Text = "Low Stock Items: " + lowItems;
            ...
            */
            ...
            using (SqlConnection conn = new SqlConnection(myConn))
            {
                ...
                SqlCommand cmd2 = new SqlCommand(
                    "SELECT ISNULL(SUM(Qty),0) FROM BillDetail " +
